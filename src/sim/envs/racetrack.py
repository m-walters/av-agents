...
def road2(env: "AVRacetrack"):
    ...
    start8 = np.array(
        [
            center4[0] + radii4 * np.cos(np.deg2rad(theta7_end)),
            center4[1] + radii4 * np.sin(np.deg2rad(theta7_end))
        ]
    )
    start8_2 = np.array(
        [
            center4[0] + (radii4 + w) * np.cos(np.deg2rad(theta7_end)),
            center4[1] + (radii4 + w) * np.sin(np.deg2rad(theta7_end))
        ]
    )
    start8_3 = np.array(
        [
            center4[0] + (radii4 + w2) * np.cos(np.deg2rad(theta7_end)),
            center4[1] + (radii4 + w2) * np.sin(np.deg2rad(theta7_end))
        ]
    )
    # start8 = np.array([
    #     end6[0] + 2 * (center4[0] - end6[0]),
    #     end6[1]
    # ])
    # start8_2 = np.array([
    #     end6_2[0] + 2 * (center4[0] - end6_2[0]),
    #     end6_2[1]
    # ])
    # start8_3 = np.array([
    #     end6_3[0] + 2 * (center4[0] - end6_3[0]),
    #     end6_3[1]
    # ])
    ...
    end8 = np.array(
        [
            42 - radii7 * np.cos(rad),
            -radii7 - radii7 * np.sin(rad)
        ]
    )
    end8_2 = np.array(
        [
            42 - (radii7 + w) * np.cos(rad),
            -radii7 - (radii7 + w) * np.sin(rad)
        ]
    )
    end8_3 = np.array(
        [
            42 - (radii7 + w2) * np.cos(rad),
            -radii7 - (radii7 + w2) * np.sin(rad)
        ]
    )
    net.add_lane(
        "h",
